...
// 🌃 ৫-রিলস ওরিজিনাল সাংহাই নাইটস লাক্সারি ক্যাসিনো র্যান্ডমাইজেশন প্রতীক তালিকা ভাই ভাই
const shanghaiNightsPool = ["SHANGHAI_DIAMOND", "GOLDEN_CAR", "PREMIUM_DRINK", "NEON_LOGO", "VIP_CARD", "GOLD_WATCH", "PARTY_MASK"];

// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে
app.get('/api/shanghai-balance', async (req, res) => {
...
// 🛫 ২. সাংহাই নাইটস ৫-রিল কোর স্লট স্পিন রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/shanghai-spin', async (req, res) => {
    ...
    try {
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.shanghai_target) ? balResponse.data.shanghai_target : null;

        let spinResults, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP ও ৫-রিলস লাক্সারি নাইট গাণিতিক লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            ...
            spinResults = [];
            for (let r = 0; r < 5; r++) {
                spinResults.push(shanghaiNightsPool[Math.floor(Math.random() * shanghaiNightsPool.length)]);
            }
            ...
            if (maxSameSymbolsCount >= 3) {
                finalStatus = "win";
                // 🚀 [ডাইনামিক কম্বো মাল্টিপ্লায়ার ওডস]: ৩টি মিললে ৪.৫০ গুণ, ৪টি মিললে ৩০ গুণ, ৫টি মিললে ২৫০ গুণের মেগা সাংহাই জ্যাকপট!
                if (maxSameSymbolsCount === 3) winMultiplier = 4.50;
                else if (maxSameSymbolsCount === 4) winMultiplier = 30.00;
                else winMultiplier = 250.00; 
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            }
            ...
            if (adminTriggeredPrize) {
                ...
                if (adminTriggeredPrize === "force_jackpot" && winMultiplier === 250.00) isLoopActive = false;
            } else {
                // ২৫০ গুণের গ্র্যান্ড সাংহাই জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে কড়া সুরক্ষায় টাইট ০.২% এ লক ভাই ভাই
                if (winMultiplier === 250.00 && Math.random() > 0.002) continue;
        ...
        console.error("Shanghai Nights Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Royal Shanghai Nights 5-Reel Slot Engine!"); });

// সাংহাই নাইটস গেম নিজস্ব কাস্টম ৫৭০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 5700; 
server.listen(PORT, () => { console.log(`🎡 Royal Shanghai Nights Engine Running on port ${PORT}`); });
